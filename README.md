...
- Commands show an abstract terminal pulse on the workstation screen while the agent remains seated and types; this is not a walking animation. Waiting agents glance aside with a question indicator. Reduced-motion settings keep both displays still.
- When a tool reports completion, failure, or interruption, an idle agent briefly shows a different reaction for each. Reactions do not interrupt work or walking; the inspector remains the source of detailed results. No reaction is inferred for an unmatched or unknown outcome.
